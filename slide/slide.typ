...
- *OpenMP*とは，共有メモリ型並列計算機用にプログラムを並列化するためのAPI仕様・標準規格
...
- 実際のHPCでは，ノード間はMPI，ノード内はOpenMPで並列化する#text(weight: "bold")[ハイブリッド並列]が重要
...
#slide(title: [スケジューリングの重要性])[
  ...
  - 特にOpenMPのfork-joinモデルでは，仕事の最も多いスレッドが終わるまで待たないといけない
...
#slide(title: [スケジューリングの種類])[
  - forループのスケジューリングの種類には以下の3通りがある
    ...
    + *`dynamic`*：スレッドごとにchunk個のイテレーションを動的に割り当てる，終わったスレッドから次のchunkを取りに行く
...
#slide(title: [dynamic])[
  - *`schedule(dynamic, CHUNK_SIZE)`*
    - ループを `CHUNK_SIZE` 個ごとに分割し，#text(weight: "bold")[処理が終わったスレッドに順々で次の処理を割り当てる]
...
#slide(title: [guided])[
  - *`schedule(guided, CHUNK_SIZE)`*
    - #text(weight: "bold")[徐々にチャンクサイズを小さくしながら，処理が終わったスレッドに順々で次の処理を割り当てる]
    - `CHUNK_SIZE` はチャンクサイズの最小単位
    - チャンクサイズに1より大きい $k$ を指定した場合，チャンクサイズは指数的に $k$ まで小さくなるが，最後のチャンクは $k$ より小さくなる場合がある
    - `CHUNK_SIZE` を省略すると1になる
...
#slide(title: [各スケジューリングの特徴])[
  ...
  - 割り当てのオーバーヘッド
    - dynamic，guidedは実行時スケジューリングのため，staticよりもオーバーヘッドが大きい
    - #text(weight: "bold")[事前に負荷分散が均衡となるスケジューリングを組むことができれば，]staticスケジューリングを用いることでさらに高速化できる可能性がある
]
...
#slide(title: [staticのチャンクサイズを変える])[
  ...
  guided : 0.259611 ± 0.067941 s
  ```
  - 静的割り当てなぶんstaticはguidedより速くなるかと思ったが，明確に速くなるわけでもなかった
...
#slide(title: [[補足] タスク並列のスケジューリング (3/3)])[
  ...
  #grid(
    ...
    [
      ...
      - 実際にfibで`OMP_NUM_THREADS`をスケールさせながら測定してみる
    ],
    [
      #align(center)[
...
#slide(title: [[補足] GPUタスク並列処理系GTaP])[
  ...
  - 2つの実行モードをサポートする
    ...
    - *1スレッドブロック1タスク*：
      タスク内部にデータ並列性がある場合に，1つのブロック内のスレッドが協調して実行する
]
...
#slide(title: [GPU利用時の注意：データ転送コスト])[
  ...
  - 小さい計算を何度もGPUに投げると，計算時間より転送・起動オーバーヘッドが支配的になりやすい
  ```c
...
#slide(title: [各種ディレクティブの実装④：atomicの実装])[
  ...
  - armにおいて `ldadd` は「メモリ位置 `[x0]` から値をロード→ `w0` の値を加算→結果を `[x0]` にストア→元のメモリ値を `w1` に返す」を*不可分で*実行する命令
]
