#!/usr/bin/env python3
"""計算律速 (measure_threads_compute) のスレッド数スイープ作図。"""
...
N = 2_000_000
WORK = 100
CSV_PATH = CSV_DIR / "threads_compute.csv"


def build() -> None:
    subprocess.run(
        ["make", "measure_threads_compute", f"N={N}", f"WORK={WORK}"],
        cwd=ROOT,
        check=True,
    )
...
def measure() -> tuple[list[int], list[float], list[float], list[float], list[float]]:
    ...
    stds: list[float] = []
    for t in THREADS:
        mean, std = measure_mean(["./measure_threads_compute", str(t)], ROOT)
        means.append(mean)
        stds.append(std)
...
def plot(
    ...
    plot_speedup_sweep(
        [(threads, speedups, speedup_stds, "compute-bound loop", "o-")],
        f"Compute-bound speedup (N={N:,}, WORK={WORK}, mean of {RUNS} runs)",
        [IMG_DIR / "threads_compute.png", SLIDE_IMG / "threads_compute.png"],
    )
